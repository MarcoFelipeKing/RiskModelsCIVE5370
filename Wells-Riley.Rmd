...
Use the <b>drop-down menu</b> and <b>sliders </b> to investigate parameter combinations. We assume 1 person is infected.
```{r}

selectInput("q", label = h4("Infection of Interest:"), 
    choices = list("SARS-2" = 25, "Measels" = 400, "Tuberculosis" = 13), 
    selected = 25)
    
selectInput("V", label = h4("Type of Room:"), 
...
selectInput("fresh_flow", label = h4("How is the room ventilated:"), 
    choices = list("Windows Open a Crack" = 2.5, 
...
sliderInput(inputId = "tmax",label = h4("How long is the Room Occupied [hrs]:"),
...
WR<-reactive({
  ...
  df <- data.frame(
    ...
    standard=N1(0.6,as.numeric(input$q),as.numeric(input$fresh_flow)*as.numeric(input$V),seq(0,input$tmax,0.1),S(),1),
    masked=N1(0.6,as.numeric(input$q),as.numeric(input$fresh_flow)*as.numeric(input$V),seq(0,input$tmax,0.1),S()*(1-as.numeric(input$maskProp)/100),1)+N1(0.6*0.5,as.numeric(input$q),as.numeric(input$fresh_flow)*as.numeric(input$V),seq(0,input$tmax,0.1),S()*(as.numeric(input$maskProp)/100),1),
    extra_ventilation=N1(0.6,as.numeric(input$q),as.numeric(input$fresh_flow)*as.numeric(input$V)*1.2,seq(0,input$tmax,0.1),S,1)
...
renderPlotly({
  ...
  add_trace(x=~time, y = ~extra_ventilation, name='Increased ventilation', mode = 'plines',
            text = ~paste('Number infected: ', round(WR()$extra_ventilation))) %>%
  add_trace(x=~time, y = ~masked, name='Masking', mode = 'plines',
            text = ~paste('Number infected: ', round(WR()$masked))) %>%
...
w <- reactive({
    a<-WR() %>%
      ...
      select(risk) %>%
      slice_tail(n=1) %>% 
      mutate(NumInfected = list(rbinom(n=100, size = 100, prob = S-risk)))
